...
Elm.SignupForm.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var withUsernameTaken = F2(function (isTaken,
   model) {
      return function () {
         var currentErrors = model.errors;
         var newErrors = _U.replace([["usernameTaken"
                                     ,isTaken]],
         currentErrors);
   ...
   model) {
      ...
      "VALIDATE") ? function () {
         var usernameAvailableAction = {_: {}
                                       ,actionType: "USERNAME_AVAILABLE"
                                       ,payload: ""};
         var usernameTakenAction = {_: {}
                                   ,actionType: "USERNAME_TAKEN"
                                   ,payload: ""};
         var url = A2($Basics._op["++"],
         "https://api.github.com/users/",
         model.username);
         var request = A2($Http.get,
         $Json$Decode.succeed(usernameTakenAction),
         url);
         var neverFailingRequest = A2($Task.onError,
         request,
         function (err) {
            return $Task.succeed(usernameAvailableAction);
         });
         return {ctor: "_Tuple2"
      ...
      "SET_PASSWORD") ? {ctor: "_Tuple2"
                        ...
                        model)
                        ,_1: $Effects.none} : _U.eq(action.actionType,
      "USERNAME_TAKEN") ? {ctor: "_Tuple2"
                          ,_0: A2(withUsernameTaken,
                          true,
                          model)
                          ,_1: $Effects.none} : _U.eq(action.actionType,
      "USERNAME_AVAILABLE") ? {ctor: "_Tuple2"
                              ,_0: A2(withUsernameTaken,
                              false,
                              model)
   ...
   model) {
      ...
      _L.fromArray([A2($Html.h1,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Sensational Signup Form")]))
                   ,A2($Html.label,
                   _L.fromArray([$Html$Attributes.$for("username-field")]),
                   ...
                   ,A2($Html.label,
                   _L.fromArray([$Html$Attributes.id("password")]),
                   ...
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("validation-error")]),
                   _L.fromArray([$Html.text(viewUsernameErrors(model))]))]));
   });
