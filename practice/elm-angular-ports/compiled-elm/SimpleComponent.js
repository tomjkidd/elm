...
Elm.SimpleComponent = Elm.SimpleComponent || {};
Elm.SimpleComponent.make = function (_elm) {
   "use strict";
   _elm.SimpleComponent = _elm.SimpleComponent || {};
   if (_elm.SimpleComponent.values)
   return _elm.SimpleComponent.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var view = F2(function (address,string) {
      ...
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.placeholder("Text to reverse")
                      ...
                      function (str) {
                         return A2($Signal.message,address,Update(str));
                      })
                      ,myStyle]),
              _U.list([]))
              ,A2($Html.div,
              _U.list([myStyle]),
              _U.list([$Html.text($String.reverse(string))]))]));
   });
   var fromJSAction = A2($Signal.map,
   function (str) {
      return Update(str);
   },
   toElm);
   ...
   app.model);
   var main = app.html;
   return _elm.SimpleComponent.values = {_op: _op
                                        ,Update: Update
